...
- `email`
- `api_token`
- `auth_type`: `api_token` for MVP, with room for `oauth_bearer` later
...
- support two explicit token sources:
  - `keychain`
  - `env_or_file`
- provide a user-facing `auth set-access --suffix ORG --token TOKEN` command
- in `keychain` mode, use service name `zendesk-mgmt` and store by derived `instance_url`
- in `env_or_file` mode, resolve `ZENDESK_ACCESS_TOKEN` first, then the global JSON file at `os.UserConfigDir()/zendesk-mgmt/auth.json`
- the global JSON file is sectioned by org suffix, and each profile currently contains only one field: `access_token`
...
- `zendesk-mgmt auth login --instance https://acme.zendesk.com --email agent@acme.com --token ...`
- `zendesk-mgmt auth whoami --instance https://acme.zendesk.com`
- `zendesk-mgmt auth logout --instance https://acme.zendesk.com`
...
- release archives for `darwin/arm64`, `darwin/amd64`, and `windows/amd64`
